...
public class PresetToEntityService implements ServicePlugin {
    ...
    private static final Comparator<PresetToEntity> PRESET_LINK_COMPARATOR = Comparator.comparing(f -> orderMap.getOrDefault(f.getClass().getCanonicalName(), Integer.MAX_VALUE));
    @Inject
    ...
    public PresetToEntity presetToEntityUpdate(PresetToEntityUpdate presetToEntityUpdate, SecurityContext securityContext) {
    ...
    public boolean presetToEntityUpdateNoMerge(PresetToEntityCreate presetToEntityCreate, PresetToEntity presetToEntity) {
        boolean update = baseclassNewService.updateBaseclassNoMerge(presetToEntityCreate, presetToEntity);
        ...
        if (presetToEntityCreate.getPreset() != null && (presetToEntity.getLeftside() == null || !presetToEntityCreate.getPreset().getId().equals(presetToEntity.getLeftside().getId()))) {
    ...
    public void validate(PresetToEntityCreate presetToEntityCreate, SecurityContext securityContext) {
    ...
    public <T extends Baseclass> T getByIdOrNull(String id, Class<T> c, List<String> batchString, SecurityContext securityContext) {
        return presetToEntityRepository.getByIdOrNull(id, c, batchString, securityContext);
    }

    public PaginationResponse<PresetToEntity> getAllPresetToEntities(PresetToEntityFiltering presetToEntityFiltering, SecurityContext securityContext) {
    ...
    public PresetToEntity presetToEntityCreate(PresetToEntityCreate presetToEntityCreate, SecurityContext securityContext) {
    ...
    private PresetToEntity presetToEntityCreateNoMerge(PresetToEntityCreate presetToEntityCreate, SecurityContext securityContext) {
        PresetToEntity presetToEntity = new PresetToEntity(presetToEntityCreate.getName(), securityContext);
    ...
    public List<Preset> getPreferredPresets(PreferedPresetRequest preferedPresetRequest, SecurityContext securityContext) {
        ...
        presetToEntityFiltering.setSort(Arrays.asList(new SortParameter("priority", SortingOrder.DESCENDING)));
        ArrayList<Baseclass> rightside = new ArrayList<>(securityContext.getTenants());
        rightside.add(securityContext.getUser());
        rightside.addAll(roleService.listAllRoles(new RoleFilter().setUsers(Collections.singletonList(securityContext.getUser())), null));
        presetToEntityFiltering.setRightside(rightside);
        List<PresetToEntity> links = presetToEntityRepository.listAllPresetToEntities(presetToEntityFiltering, securityContext).parallelStream().filter(f -> f.getLeftside() != null).sorted(PRESET_LINK_COMPARATOR.thenComparing(f -> f.getPriority())).collect(Collectors.toList());
        for (PresetToEntity presetToEntity : links) {
            String canonicalName = presetToEntity.getLeftside().getClass().getCanonicalName();
        ...
        return map.values().parallelStream().flatMap(List::stream).map(f -> f.getLeftside()).collect(Collectors.toList());
